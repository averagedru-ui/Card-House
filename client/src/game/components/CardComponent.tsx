...
export const CardComponent: React.FC<CardComponentProps> = ({ card, onClick, selected, small, faceDown, disabled }) => {
  ...
  const baseClasses = `${sizeClasses} rounded-xl border-2 flex flex-col overflow-hidden shadow-md select-none relative`;
  ...
  if (card.type === 'wildcard') {
    ...
    // Solid bg color classes (not gradient) for split half rendering
    const solidBgMap: Record<PropertyColor, string> = {
      brown: 'bg-amber-800',
      blue: 'bg-blue-600',
      green: 'bg-green-600',
      red: 'bg-red-600',
    ...
    return (
      <div
        className={`${baseClasses} ${interactiveClasses} ${selectedClasses} border-yellow-400/80 overflow-hidden`}
        onClick={disabled ? undefined : onClick}
        style={isMultiColor
          ? { background: 'linear-gradient(135deg, #eab308, #22c55e, #3b82f6)' }
          : isDualColor
            ? {}
            : { background: colorMap[colors[0]] || '#4b5563' }
        }
      >
        {/* Split diagonal background for dual-color wilds */}
        {isDualColor && (
          <div className="absolute inset-0 flex" style={{ zIndex: 0 }}>
            {/* Left half */}
            <div className={`w-1/2 h-full ${solidBgMap[colors[0]]}`} />
            {/* Right half */}
            <div className={`w-1/2 h-full ${solidBgMap[colors[1]]}`} />
            {/* Diagonal divider */}
            <div className="absolute inset-0" style={{
              background: 'linear-gradient(135deg, transparent 49.5%, rgba(255,255,255,0.4) 49.5%, rgba(255,255,255,0.4) 50.5%, transparent 50.5%)'
            }} />
          </div>
        )}

        <div className={`relative z-10 bg-yellow-500 text-yellow-900 text-center py-0.5 font-bold uppercase tracking-wide ${small ? 'text-[9px]' : 'text-[10px]'}`}>
          Wild
        </div>
        <div className="relative z-10 flex-1 flex flex-col items-center justify-center px-1 gap-0.5">
          {isMultiColor ? (
            <div className={`text-white font-bold drop-shadow ${small ? 'text-[10px]' : 'text-xs'} text-center`}>Any Color</div>
          ) : isDualColor ? (
            <div className="flex flex-col items-center gap-0.5 w-full">
              <div className={`text-white font-bold drop-shadow text-center w-full ${small ? 'text-[9px]' : 'text-[10px]'}`} style={{ textShadow: '0 1px 3px rgba(0,0,0,0.8)' }}>
                {PROPERTY_SETS[colors[0]].label}
              </div>
              <div className={`text-white/70 ${small ? 'text-[8px]' : 'text-[9px]'} font-medium`}>or</div>
              <div className={`text-white font-bold drop-shadow text-center w-full ${small ? 'text-[9px]' : 'text-[10px]'}`} style={{ textShadow: '0 1px 3px rgba(0,0,0,0.8)' }}>
                {PROPERTY_SETS[colors[1]].label}
              </div>
            </div>
          ) : colors.map(c => (
        ...
        <div className={`relative z-10 bg-black/40 text-white text-center py-0.5 font-mono ${small ? 'text-[9px]' : 'text-[10px]'}`}>
          ${card.value}M
        </div>
      </div>
    );
