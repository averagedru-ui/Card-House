...
export const GameOverScreen: React.FC = () => {
  ...
  useEffect(() => {
    ...
    recordGameResult(isHumanWinner, { turnNumber, playerCount: players.length, bankValue, propertyColors });
  }, []);

  // Delay showing the full screen so the board is visible for a moment
  useEffect(() => {
  ...
  return (
    <div className="min-h-[100dvh] bg-gradient-to-br from-gray-900 via-indigo-950 to-gray-900 flex flex-col items-center justify-center p-4 relative overflow-hidden">
      {/* Winner announcement — shows immediately */}
      <motion.div
      ...
      >
        <div className="text-7xl mb-3">{isHumanWinner ? '🏆' : '🎉'}</div>
        ...
        <p className="text-gray-400 text-base">
          {isHumanWinner
            ...
            : `${winnerPlayer?.name} collected 3 complete sets`}
        </p>
      </motion.div>

      {/* Standings + buttons — delayed reveal */}
      <motion.div
      ...
      >
        <div className="bg-gray-800/80 rounded-2xl p-4 border border-gray-700 mb-6">
        ...
        <div className="flex gap-3 justify-center">
          <button
            onClick={() => { useProfile.getState().resetGameFlags(); startGame(players.length, playerName); }}
